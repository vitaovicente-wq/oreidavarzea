...
const elements = {
  ...
  linkProximaTela: document.getElementById('linkProximaTela'),
...
function updateCash() { elements.cashDisplay.textContent = formatReal(financas.caixaAtual); }
...
function renderHealthBar(health) { return `<div class="health-bar"><div class="health-fill" style="width:${health}%"></div></div>`; }
function showPlayerSpeech(playerName, speech) { elements.playerSpeechBox.innerHTML = `💬 <span class="player-name-highlight">${playerName}</span>: "${speech}"`; }
...
function generateSkillDistribution() { 
    ...
    for (let i = 1; i < TOTAL_MARKET; i++) { 
        ...
        skills.push(Math.min(100, skill)); 
    } 
...
function calculatePlayerValue(skill, age) { 
    ...
    let ageMultiplier = 1; 
    if (age <= 20) ageMultiplier = 1.8; 
    else if (age <= 25) ageMultiplier = 1.5; 
    ...
    const baseSalario = 30 + (skill * 0.8); 
    const salario = Math.round((baseSalario + Math.random() * 20) / 5) * 5; 
...
function generatePlayers() {
    ...
    const counts = { 'Goleiro': 6, 'Lateral': 8, 'Zagueiro': 8, 'Volante': 6, 'Meia': 8, 'Atacante': 6 };
    ...
    positions.forEach(pos => {
        const footBucket = createFootDistribution(counts[pos]);
        for (let i = 0; i < counts[pos]; i++) {
            ...
            const health = 100;
            const isPai = Math.random() < 0.2;
            const valores = calculatePlayerValue(skill, age);
            const apresentacoes = { Goleiro: "Pode bater, professor! Pênalti pra mim é doce!", Lateral: "Sou o motorzinho da lateral!", Zagueiro: "Aqui na zaga, ninguém passa!", Volante: "Corro por mim e pelo time todo!", Meia: "O jogo passa pelos meus pés!", Atacante: "Na frente do gol, eu não perdoo!" };
            players.push({
                id: 'p' + id, name, pos,
                foot: footBucket.pop() || 'Direito',
                age, skill, health, isPai,
...
function renderMarket(filter) {
    ...
    list.forEach(p => {
        ...
        tr.innerHTML = `<td><span class="player-name-highlight">${p.name}</span><div class="muted">${p.pos}</div></td><td>${p.pos}</td><td>${p.foot}</td><td>${p.age}</td><td>${formatReal(p.passe)}</td><td>${formatReal(p.salarioJogo)}</td><td><select class="contrato-select" data-id="${p.id}" ${isHired ? 'disabled' : ''}><option>3 meses</option><option selected>6 meses</option><option>1 ano</option></select></td><td>${renderSkillBar(p.skill)} ${p.skill}%</td><td>${renderHealthBar(p.health)} ${p.health}%</td><td>${p.profissao.nome}${p.isPai ? ' <span class="badge-pai">Pai</span>' : ''}</td><td><input type="checkbox" data-id="${p.id}" ${isHired ? 'checked' : ''}></td>`;
        tr.querySelector('.player-name-highlight').addEventListener('mouseenter', () => showPlayerSpeech(p.name, p.apresentacao));
        tr.querySelector('.contrato-select').addEventListener('change', (e) => {
            const player = players.find(pl => pl.id === e.target.dataset.id);
            if (player) player.contrato = e.target.value;
        });
        tr.querySelector('input[type=checkbox]').addEventListener('change', onToggleHire);
...
function onToggleHire(e) {
    ...
    if (cb.checked) {
        ...
        if (financas.caixaAtual - ply.passe < 0) { showNotif('Saldo insuficiente!'); cb.checked = false; return; }
        financas.caixaAtual -= ply.passe;
        hired.push({...ply});
    } else {
        financas.caixaAtual += ply.passe;
...
function updateCheckboxStates() {
    elements.playersTbody.querySelectorAll('input[type=checkbox]').forEach(cb => {
        const ply = players.find(p => p.id === cb.dataset.id);
        cb.disabled = !cb.checked && (hired.length >= MAX_PLAYERS || financas.caixaAtual - ply.passe < 0);
    });
}
...
function updateButtonStates() {
    elements.btnVoltarStep.style.display = marketStep > 1 ? 'inline-block' : 'none';
    elements.btnEscalar.disabled = hired.length !== MAX_PLAYERS;
    if (marketStep >= 1 && marketStep < steps.length) {
        const current = steps[marketStep];
        elements.btnNextStep.disabled = countHiredByPos(current.filter) < current.min;
    }
    updateSelectedCount();
    updateCash();
}
...
elements.btnNextStep.addEventListener('click', () => {
    ...
    if (current && current.filter) {
        if (countHiredByPos(current.filter) < current.min) {
            return showNotif(`Escolha no mínimo ${current.min} ${current.filter}(s)!`);
        }
    }
    marketStep++;
...
elements.linkProximaTela.addEventListener('click', (event) => {
  if (elements.btnEscalar.disabled) {
    event.preventDefault();
    showNotif('Você precisa contratar exatamente 22 jogadores!');
    return;
  }
  const gastoNestaSessao = initialCash - financas.caixaAtual;
  financas.gastosContratacoes += gastoNestaSessao;
...
function init() {
  ...
  if(isNewGame){
    financas = {
      caixaAtual: 5000, gastosContratacoes: 0, gastosSalarios: 0, 
      gastosBicho: 0, receitaPartidas: 0, receitaPremiosPatrocinios: 0
    };
    hired = [];
    localStorage.setItem('financasDoTime', JSON.stringify(financas));
    localStorage.setItem('elencoDoTime', JSON.stringify(hired));
  ...
  initialCash = financas.caixaAtual;
