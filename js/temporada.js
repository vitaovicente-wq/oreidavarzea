...
function generateSeasonFixtures(universe, userTeamName) {
    ...
    const giganesDoBairro = universe.map(t => t.name);
    ...
    // Sorteia e cria a Copa 1º de Maio para a primeira temporada
    const copaTeams = [...torneioTeams].sort(() => 0.5 - Math.random()).slice(0, 8);
    for (let i = 0; i < copaTeams.length; i += 2) {
        season.copa.schedule.push({ home: copaTeams[i], away: copaTeams[i+1], played: false });
    }
