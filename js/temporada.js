...
function createUniverse() {
    const teamNames = [ "Tsunami da ZL", "Galácticos do Grajaú", "Ajax da Vila Sônia", "Real Madruga", "Mulekes da Vila", "Fúria do Capão Redondo", "EC Beira-Rio", "Juventus da Mooca", "Parma da Augusta", "Boca do Lixo FC", "Manchester Paulista", "PSV - Pau Sem Vontade" ];
...
function generateSeasonFixtures(universe, userTeamName) {
    ...
    const torneioTeams = [userTeamName, ...universe.slice(0, 7).map(t => t.name)];
    const shuffledTeams = [...torneioTeams].sort(() => 0.5 - Math.random());
    season.torneio.groups['A'] = shuffledTeams.slice(0, 4);
    season.torneio.groups['B'] = shuffledTeams.slice(4, 8);
    ...
    Object.values(season.torneio.groups).forEach(group => {
        schedule.push({ week: 1, home: group[0], away: group[1], played: false });
        schedule.push({ week: 1, home: group[2], away: group[3], played: false });
        schedule.push({ week: 2, home: group[0], away: group[2], played: false });
        schedule.push({ week: 2, home: group[3], away: group[1], played: false });
        schedule.push({ week: 3, home: group[0], away: group[3], played: false });
        schedule.push({ week: 3, home: group[1], away: group[2], played: false });
    });
    season.torneio.schedule = schedule;
    return season;
}
...
function simulateAIMatch(homeTeamName, awayTeamName) {
    ...
    if(!homeTeam || !awayTeam) return { homeTeam, awayTeam, homeScore: 0, awayScore: 0};
...
function processLastMatchResult() {
    ...
    if(matchIndex > -1) {
        ...
        const allPlayed = seasonData.torneio.schedule.filter(m => m.week === weekOfLastMatch).every(m => m.played);
        if(allPlayed) seasonData.currentWeek++;
    }
...
function displayNextMatch() {
    ...
    if (nextMatch) {
        ...
        elements.nextMatchInfo.innerHTML = `<div class="vs">${nextMatch.home} vs ${nextMatch.away}</div><p class="match-details-small">${location}</p>`;
        elements.linkPlayMatch.onclick = () => {
        ...
        elements.nextMatchInfo.innerHTML = `<p>Aguardando o mata-mata...</p>`;
