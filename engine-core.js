...
// ATUALIZADO: Recuperação de Lesão + Integração com Treino
...
window.Engine = {
    ...
    novoJogo: function(pais, divisao, nomeTimeSelecionado) {
        ...
        // ... Geração de Times (Mantida igual) ...
        let timesDaLiga = [];
        ...
        let orcamento = mediaOvr > 80 ? 15000000 : (mediaOvr > 70 ? 6000000 : 2000000);
        ...
        const estado = {
            ...
            flags: { boasVindasLida: false, patroEnviado: false, tvEnviado: false, treinoAtual: 'balanceado' }, // Flag de treino adicionada
        ...
        setTimeout(() => { if(window.Engine.Contratos) window.Engine.Contratos.enviarBoasVindas(window.Engine.carregarJogo()); }, 200);
    },
    ...
    atualizarTabela: function(estado) {
        ...
        estado.calendario.forEach(rod => { rod.jogos.forEach(jogo => { if(jogo.jogado) this._computar(tab, jogo); }); });
        ...
        if(rodadaJogada > 0 && estado.recursos.ultimaRodadaProcessada < rodadaJogada) {
            ...
            // 1. Processar Recuperação de Lesões (CRUCIAL)
    ...
    // Nova função interna para tratar lesões
            ...
            time.elenco.forEach(jogador => {
                if (jogador.status === "Lesionado" && jogador.rodadasFora > 0) {
                    ...
                    if (jogador.rodadasFora <= 0) {
                        ...
                        // Avisa se for do time do jogador
                        if (time.nome === estado.info.time && this.Sistema) {
                            this.Sistema.novaMensagem(
                                "Recuperação", 
                                `<p>O jogador <b>${jogador.nome}</b> recebeu alta do Dr. Tairo e está apto para jogar.</p>`, 
                                "dm", "DM"
                            );
                        }
                    }
